const BOOK_ALIASES: Record<string, string> = {
  창세기: "창세기", 출애굽기: "출애굽기", 레위기: "레위기", 민수기: "민수기",
  신명기: "신명기", 여호수아: "여호수아", 사사기: "사사기", 룻기: "룻기",
  사무엘상: "사무엘상", 사무엘하: "사무엘하", 열왕기상: "열왕기상",
  열왕기하: "열왕기하", 역대상: "역대상", 역대하: "역대하", 에스라: "에스라",
  느헤미야: "느헤미야", 에스더: "에스더", 욥기: "욥기", 시편: "시편",
  잠언: "잠언", 전도서: "전도서", 아가: "아가", 이사야: "이사야",
  예레미야: "예레미야", 예레미야애가: "예레미야애가", 에스겔: "에스겔",
  다니엘: "다니엘", 호세아: "호세아", 요엘: "요엘", 아모스: "아모스",
  오바댜: "오바댜", 요나: "요나", 미가: "미가", 나훔: "나훔",
  하박국: "하박국", 스바냐: "스바냐", 학개: "학개", 스가랴: "스가랴",
  말라기: "말라기", 마태복음: "마태복음", 마가복음: "마가복음",
  누가복음: "누가복음", 요한복음: "요한복음", 사도행전: "사도행전",
  로마서: "로마서", 고린도전서: "고린도전서", 고린도후서: "고린도후서",
  갈라디아서: "갈라디아서", 에베소서: "에베소서", 빌립보서: "빌립보서",
  골로새서: "골로새서", 데살로니가전서: "데살로니가전서",
  ...
  잠: "잠언", 전: "전도서", 사: "이사야", 렘: "예레미야", 겔: "에스겔",
  단: "다니엘", 호: "호세아", 암: "아모스", 욘: "요나", 미: "미가",
...
export function parseBibleReference(input: string): BibleRef | null {
  ...
  const colonMatch = normalized.match(
    /^([가-힣0-9]+)\s*(\d{1,3})\s*:\s*(\d{1,3})(?:\s*[-~]\s*(\d{1,3}))?$/
  );
  if (colonMatch) {
    ...
    return {
      book,
      chapter: Number(colonMatch[2]),
      verseStart: Number(colonMatch[3]),
      verseEnd: Number(colonMatch[4] || colonMatch[3]),
    };
  }
  ...
  return {
    book,
    chapter: Number(chapterVerseMatch[2]),
    verseStart: Number(chapterVerseMatch[3]),
    verseEnd: Number(chapterVerseMatch[4] || chapterVerseMatch[3]),
  };
}

export function extractBibleReferences(
  text: string,
  maxRefs = 3
): BibleRef[] {
  const matches =
